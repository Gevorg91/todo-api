...
describe("End to End Integration Tests Suite For Workspaces Flow", () => {
  ...
  beforeEach(async () => {
    const tigranRegistrationResponse = await request(testApp)
    ...
    const gevRegistrationResponse = await request(testApp)
      .post("/api/users/register")
      .send({
  ...
  it("should not create a new workspace without an access token", async () => {
    ...
    // Act
    const res = await request(testApp).post("/api/workspaces/").send({
      name: "My First Workspace",
    });
  ...
  it("should get unauthorised when trying to access wrong Workspace", async () => {
    ...
    // Gevor
    expect(gevorWorkspacesResponse.header["content-type"]).toBe(
      "application/json; charset=utf-8"
    );
    expect(gevorWorkspacesResponse.statusCode).toBe(200);
    expect(gevorWorkspacesResponse.body.length).toBe(1);
    expect(gevorWorkspacesResponse.body[0].name).toBe(
      "My First Workspace created By Gevor"
    );
    ...
    // Tigran
    expect(tigranWorkspacesResponse.header["content-type"]).toBe(
      "application/json; charset=utf-8"
    );
    expect(tigranWorkspacesResponse.statusCode).toBe(200);
    expect(tigranWorkspacesResponse.body.length).toBe(1);
    expect(tigranWorkspacesResponse.body[0].name).toBe(
      "My First Workspace created By Tigran"
    );
    ...
    // Tigran Unauthorised
    expect(tigranUnauthorisedWorkspacesResponse.header["content-type"]).toBe(
      "application/json; charset=utf-8"
    );
  ...
  it("should add Gev to Tigran's Workspace as a member", async () => {
    ...
    // Assert
    expect(
      tigranWorkspacesResponseAfterGevAddition.header["content-type"]
    ).toBe("application/json; charset=utf-8");
    expect(
      tigranWorkspacesResponseAfterGevAddition.body[0].members.length
    ).toBe(2);
  });
});
